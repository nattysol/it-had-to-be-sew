...
export default function OrderWizard() {
  ...
  useEffect(() => {
    const query = `{
      ...
      "threads": *[_type == "inventory" && category == "thread"]{_id, name, "imageUrl": image.asset->url}
    }`
    ...
    <div className="bg-[#f8f7fa] min-h-screen font-sans text-[#131118] pb-64">
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-slate-100 pt-4 pb-0">
        <div className="flex items-center justify-between max-w-xl mx-auto px-6 mb-3">
          ...
          <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">Step {step} of 4</h2>
          <div className="w-10"></div>
        </div>
        <div className="w-full h-1 bg-slate-100">
          <motion.div className="h-full bg-[#652bee]" initial={{ width: 0 }} animate={{ width: `${(step / 4) * 100}%` }} transition={{ duration: 0.5 }} />
        </div>
      </div>
            ...
            <motion.div key="step1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                {threads.map((t) => (
                    <div 
                        key={t._id}
                        onClick={() => updateDesign('threadColor', t.name)}
                        className={`rounded-xl border-2 p-2 cursor-pointer text-center transition-all ${
                            designDetails.threadColor === t.name
                            ? 'border-[#652bee] bg-[#652bee]/5'
                            : 'border-white bg-white hover:border-slate-200'
                        }`}
                    >
                        <div className="size-12 mx-auto rounded-full bg-slate-100 mb-2 overflow-hidden relative border border-slate-100">
                            {t.imageUrl ? (
                                <img src={t.imageUrl} alt={t.name} className="w-full h-full object-cover" />
                            ) : (
                                <span className="material-symbols-outlined text-slate-400 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">spool</span>
                            )}
                        </div>
                 ...
                 >
                    <span className="material-symbols-outlined text-slate-400 mb-1">palette</span>
                    <p className="text-[10px] font-bold">Artist Choice</p>
                 </div>
              </div>
            ...
            <motion.div key="step2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              ...
              <div className="space-y-4 mb-8">
                {BATTING_OPTIONS.map((b) => (
                  <div key={b.id} onClick={() => selectBatting(b)} className={`p-5 rounded-2xl border-2 cursor-pointer flex items-center gap-4 transition-all ${selectedBatting?.id === b.id ? 'border-[#652bee] bg-[#652bee]/5 shadow-md' : 'border-transparent bg-white shadow-sm hover:border-slate-200'}`}>
              ...
              <div className="flex gap-4 mb-6">
                <div className="flex-1"><label className={labelClass}>Width (in)</label><input type="number" className={inputClass} value={backing.width || ''} onChange={(e) => setBacking(Number(e.target.value), backing.height)} /></div>
                <div className="flex-1"><label className={labelClass}>Height (in)</label><input type="number" className={inputClass} value={backing.height || ''} onChange={(e) => setBacking(backing.width, Number(e.target.value))} /></div>
              </div>
            </motion.div>
          )}
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 space-y-6 mb-8 shadow-sm">
                ...
                {trimming.wanted && (
                  <div className="pt-4 border-t border-slate-100 space-y-4 animate-in slide-in-from-top-2">
                    <div><label className={labelClass}>Trimming Method</label><select className={inputClass} value={trimming.method} onChange={(e) => updateTrimming('method', e.target.value)}><option>To Edge</option><option>1/4 inch beyond</option><option>Custom</option></select></div>
                    <div><label className={labelClass}>Scraps</label><select className={inputClass} value={trimming.returnScraps} onChange={(e) => updateTrimming('returnScraps', e.target.value)}><option>No, discard them</option><option>Return Fabric Only</option><option>Return Fabric & Batting</option></select></div>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 space-y-6 shadow-sm">
                <div className="flex justify-between items-center"><span className="font-bold text-lg text-slate-800">Add Binding?</span><input type="checkbox" className="size-6 accent-[#652bee]" checked={binding.wanted} onChange={(e) => updateBinding('wanted', e.target.checked)} /></div>
                {binding.wanted && (
                  <div className="pt-4 border-t border-slate-100 space-y-4 animate-in slide-in-from-top-2">
                     <div><label className={labelClass}>Binding Method</label><select className={inputClass} value={binding.method} onChange={(e) => updateBinding('method', e.target.value)}><option>Machine (Sturdy)</option><option>Hand (Invisible/Show)</option></select></div>
                     <div><label className={labelClass}>Strip Width</label><input type="text" className={inputClass} value={binding.stripWidth} onChange={(e) => updateBinding('stripWidth', e.target.value)} placeholder="e.g. 2.5 inches" /></div>
                  </div>
                )}
              </div>
            </motion.div>
            ...
            <motion.div key="step4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                     <div><label className={labelClass}>First Name</label><input placeholder="Jane" className={inputClass} value={customer.firstName} onChange={(e) => updateCustomer('firstName', e.target.value)} /></div>
                     <div><label className={labelClass}>Last Name</label><input placeholder="Doe" className={inputClass} value={customer.lastName} onChange={(e) => updateCustomer('lastName', e.target.value)} /></div>
                  </div>
                  <div><label className={labelClass}>Email Address</label><input type="email" placeholder="jane@example.com" className={inputClass} value={customer.email} onChange={(e) => updateCustomer('email', e.target.value)} /></div>
                  <div><label className={labelClass}>Phone Number</label><input type="tel" placeholder="(555) 123-4567" className={inputClass} value={customer.phone} onChange={(e) => updateCustomer('phone', e.target.value)} /></div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-6 space-y-4">
                  <div><label className={labelClass}>Street Address</label><input placeholder="123 Quilt Lane" className={inputClass} value={customer.address} onChange={(e) => updateCustomer('address', e.target.value)} /></div>
                  <div className="flex gap-4">
                     <div className="flex-[2]"><label className={labelClass}>City</label><input placeholder="Las Vegas" className={inputClass} value={customer.city} onChange={(e) => updateCustomer('city', e.target.value)} /></div>
                     <div className="flex-1"><label className={labelClass}>State</label><select className={`${inputClass} px-2`} value={customer.state} onChange={(e) => updateCustomer('state', e.target.value)}><option value="" disabled>NV</option>{US_STATES.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                     <div className="w-24"><label className={labelClass}>Zip</label><input placeholder="89101" className={inputClass} value={customer.zip} onChange={(e) => updateCustomer('zip', e.target.value)} /></div>
                  </div>
              </div>
              <label className="flex items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 cursor-pointer"><input type="checkbox" className="size-5 accent-[#652bee]" checked={consent.socialMedia} onChange={(e) => updateConsent('socialMedia', e.target.checked)} /><span className="text-sm font-bold text-slate-600">I allow photos of my quilt on social media.</span></label>
              {errorMessage && <div className="mt-4 p-4 bg-red-50 text-red-600 rounded-xl text-sm font-bold text-center animate-pulse">{errorMessage}</div>}
            </motion.div>
          )}
        </AnimatePresence>
      </main>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 px-4 pb-6 pt-4 bg-gradient-to-t from-[#f8f7fa] via-[#f8f7fa] to-transparent">
        <div className="max-w-xl mx-auto bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden ring-1 ring-black/5">
          <div className="px-6 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div className="flex items-center gap-2"><span className="material-symbols-outlined text-[#652bee] text-lg">receipt_long</span><span className="font-bold text-sm text-slate-800">Live Estimate</span></div>
          </div>
          <div className="px-6 py-4 space-y-2">
            <div className="flex justify-between text-sm"><span className="text-slate-500 font-medium truncate max-w-[200px]">{selectedPattern?.title || 'Edge-to-Edge'} ({dimensions.width || 0}"x{dimensions.height || 0}")</span><span className="font-bold text-slate-800 font-mono">${(dimensions.width * dimensions.height * 0.025).toFixed(2)}</span></div>
            {(selectedBatting || binding.wanted) && (
                <div className="flex flex-col text-xs text-slate-400 font-medium pl-3 border-l-2 border-[#652bee]/30 gap-1.5 pt-1">
                    {selectedBatting && <div className="flex justify-between"><span>{selectedBatting.name}</span><span>+${(dimensions.width * dimensions.height * selectedBatting.price).toFixed(2)}</span></div>}
                    {binding.wanted && <div className="flex justify-between"><span>Binding Service</span><span>+${((dimensions.width + dimensions.height) * 2 * 0.25).toFixed(2)}</span></div>}
                </div>
            )}
            <div className="flex justify-between items-end pt-3 border-t border-slate-100 mt-2"><span className="font-bold text-slate-900">Estimated Total</span><span className="font-bold text-[#652bee] text-2xl leading-none">${estimatedTotal.toFixed(2)}</span></div>
          </div>
          <div className="p-4 pt-0">
            {step < 4 ? (
               <button onClick={nextStep} className="w-full bg-[#131118] text-white font-bold py-4 rounded-xl flex items-center justify-center gap-3 shadow-lg hover:bg-[#652bee] transition-all transform active:scale-[0.98]">Next Step <span className="material-symbols-outlined text-sm">arrow_forward</span></button>
            ) : (
               <button onClick={handleSubmit} disabled={isSubmitting} className={`w-full text-white font-bold py-4 rounded-xl flex items-center justify-center gap-3 shadow-lg transition-all transform active:scale-[0.98] ${isSubmitting ? 'bg-slate-400 cursor-not-allowed' : 'bg-[#652bee] hover:bg-[#5423c9]'}`}>
                 {isSubmitting ? <><span className="material-symbols-outlined animate-spin">sync</span>Processing...</> : <>Submit Order <span className="material-symbols-outlined">check</span></>}
               </button>
            )}
          </div>
        </div>
...
function SuccessView() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8f7fa] p-6 text-center animate-in zoom-in duration-300">
      ...
      <button onClick={() => window.location.href = '/'} className="bg-[#131118] text-white px-8 py-4 rounded-xl font-bold hover:bg-[#652bee] transition-colors shadow-lg">Return Home</button>
    </div>
  )
}