...
import { updateOrderStatus, updateOrderTime, deductMaterialUsage } from '../actions';
...
export const ProjectWorkspaceModal = ({ order, isOpen, onClose }: ProjectWorkspaceModalProps) => {
  ...
  // Calculate Profit Metrics locally
  ...
  // MAIN BUTTON HANDLER (Start / Finish / Restart)
  const handleMainAction = () => {
    if (!order?.id) return;
    const currentStatus = order.status.toLowerCase();

    // If "In Progress", we need to finish => Show Weight Prompt first
    if (currentStatus === 'in-progress' || currentStatus === 'in progress') {
  ...
  // FINAL FINISH LOGIC (After Weighing)
  const confirmFinish = () => {
    if (!order?.id) return;
    ...
    startTransition(async () => {
        ...
        // 3. Deduct Inventory
        // Note: In a real implementation, you'd find the Inventory ID for the 'thread' used in this order.
        // For now, we simulate the logic. You would likely store 'inventoryId' on the order or look it up.
        if (weightUsed) {
           // await deductMaterialUsage("SPECIFIC_INVENTORY_ID", Number(weightUsed));
           console.log(`Deducting ${weightUsed}g from inventory logic...`);
        }
    ...
    <>
        ...
        >
          ...
          <div className="w-full md:w-1/3 bg-slate-50 dark:bg-[#151022] border-r border-slate-100 dark:border-[#2d2445] flex flex-col overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              {/* Conditional Info: Completed (Customer) vs Active (Materials) */}
              {isCompleted ? (
                  <div>
                      ...
                      <div className="bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 p-4 space-y-3 text-sm">
                          <div className="flex items-start gap-3">
                              ...
                              <span className="font-semibold break-all">{order.customer?.email || 'N/A'}</span>
                          </div>
                          <div className="flex items-start gap-3">
             ...
             <div className="p-8 overflow-y-auto flex-1">
                ...
                {isCompleted && financials ? (
                    <div className="space-y-8 animate-in slide-in-from-bottom-4">
                        ...
                        {/* Materials Used */}
                        <div>
                            <h2 className="text-lg font-bold font-serif mb-4">Production Data</h2>
                            <div className="grid grid-cols-2 gap-4">
                               <div className="p-4 border rounded-xl flex justify-between items-center">
                                   <span className="text-slate-500 font-bold text-sm">Actual Time</span>
                               ...
                               <div className="p-4 border rounded-xl flex justify-between items-center">
                                   ...
                                   {/* Simple estimate or display field if saved */}
                  ...
                  <div className="space-y-8 animate-in fade-in">
                      ...
                      <div className="bg-slate-900 text-white p-6 rounded-2xl flex items-center justify-between shadow-lg">
                          ...
                          <div className={`size-12 rounded-full bg-white/10 flex items-center justify-center ${isTimerRunning ? 'animate-pulse' : ''}`}>
      ...
      {showFinishPrompt && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in">
            <div className="bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 text-center">
                <div className="size-16 bg-[#652bee]/10 text-[#652bee] rounded-full flex items-center justify-center mx-auto mb-4">
                    <span className="material-symbols-outlined text-3xl">scale</span>
                </div>
                <h3 className="text-xl font-bold font-serif mb-2">Weigh Your Thread</h3>
                <p className="text-slate-500 mb-6 text-sm">
                  Weigh the spool now. Enter the difference from start weight to auto-deduct inventory.
                </p>
                ...
                <div className="flex items-center justify-center gap-2 mb-6">
                    <input 
                      type="number" 
                      autoFocus
                      className="w-24 p-3 text-center text-3xl font-bold border-b-2 border-[#652bee] outline-none text-[#652bee]"
                      placeholder="0"
                      value={weightUsed}
                      onChange={e => setWeightUsed(e.target.value)}
                    />
                    <span className="font-bold text-slate-400">grams used</span>
                </div>

                <div className="grid grid-cols-2 gap-3">
                    <button 
                        onClick={() => { setShowFinishPrompt(false); setIsTimerRunning(true); }} 
                    ...
                    <button 
                        ...
                        disabled={!weightUsed}
                        className="bg-[#652bee] text-white py-3 rounded-xl font-bold shadow-lg hover:bg-[#5423c9] disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Complete Job
                    </button>
                </div>
